...
`:Gdiff` : Opens a 3 way diff where the :

    * The left window contains the version from the target branch ( Current branch
    where we are working ex :`git merge branch` from master branch, in this case
    master is the target )
    * The middle window contains the working copy of the file complete with
    conflict markers
    * The right window contains the version from the merge branch ( Branch that
    we are doing the merge with ex : `git merge branch`, in this case branch will
    be the merge branch )

`:diffget {version buffer}` : If the cursor is over a conflicted area in the working copy
vim will select the version from the specified version. By general rule :

    * `:diffget //2` : Pulls the target version ( Left )
    * `:diffget //3` : Pulls the merge version ( Right )

`:diffput {working file buffer}` / `:dp` : If the cursor is over a conflicted area in one
of the branch buffers this will replace the working copy segment for the buffer
we are on

`:Gwrite!` : If over a branch buffer this will overrite the working copy completly
and close the 3-way split

`:diffupdate` : Sycs the buffers to the working copy ( for visual clarity )

`:only` : Closes the 3-way split and keeps the working file open

#### Sortcuts

`[c` : Jumps to previous conflict
